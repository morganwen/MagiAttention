...
"""Multi-rank CP tests for the Magi_DSA V4 runtime (reference backend).

CP=2 against a CP=1 full-sequence run with identical weights, for the
...
SQ_GLOBAL = 512
HIDDEN = 256
SEG_LENS = [37, 128, 200, 3, 96, 48]  # ragged; sample 2 spans the CP cut
...
class TestDSAV4CP(DistTestBase):
    @property
    ...
    def _inputs(self, ratio, flat: bool):
        ...
        x = torch.randn(SQ_GLOBAL, *shape_b, HIDDEN, dtype=torch.bfloat16, device="cuda")
        qr = torch.randn(SQ_GLOBAL, *shape_b, 64, dtype=torch.bfloat16, device="cuda")
        q = torch.randn(SQ_GLOBAL, *shape_b, 16, 64, dtype=torch.bfloat16, device="cuda")
        kv = torch.randn(SQ_GLOBAL, *shape_b, 64, dtype=torch.bfloat16, device="cuda")
        sink = torch.randn(16, dtype=torch.float32, device="cuda")
        g = torch.randn(SQ_GLOBAL, *shape_b, 16 * 64, dtype=torch.bfloat16, device="cuda")
    ...
    def _run_case(self, ratio, packed: bool):
        ...
        assert abs(kl_sum.item() - kl_ref.item()) <= 5e-5, (
        ...
        assert _rel_ok(sink_g, sink_ref.grad, rtol=1e-2, atol=1e-4), (
        ...
        for (n, p), (_, p_ref) in zip(
            ...
            if ref_grad.abs().max() > 1e-6:
                assert _sim_ok(grad, ref_grad, thres=2e-3), (
                    f"ratio={ratio} packed={packed}: param {n}"
                )

    @skip_if_lt_x_gpu(1)
    @with_comms
    def test_single_sample_cp2(self):
        for ratio in (0, 4, 128):
            self._run_case(ratio, packed=False)

    @skip_if_lt_x_gpu(1)
    @with_comms
    def test_packed_cp2(self):
        for ratio in (0, 4, 128):
